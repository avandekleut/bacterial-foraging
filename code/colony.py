...
def simulate(J, S=10, p=2, c=0.1, N_c=100, rng=(-10,10), d_attract=0.1, w_attract=0.2, h_repellant=0.1, w_repellant=10):
    ...
    for j in range(N_c):
        for i in range(S):
            ...
            theta_histories[i].append(theta[i].copy())
...
#         return result
#
#     J_histories = np.zeros((N_c, S))
#     theta_histories = np.zeros((N_c, S, p))
#
#     for j in range(N_c):
#         for i in range(S):
#             phi = np.random.uniform(low=-1, high=1, size=p)
...
#             theta[i] = theta[i] + c*phi
#
#             theta_histories[j, i] = theta[i].copy()
#             J_histories[j, i] = J(theta[i])
#
#             while J(theta[i] + c*phi) + J_cc(theta[i] + c*phi) < J(theta[i]) + J_cc(theta[i]):
#                 theta[i] = theta[i] + c*phi
#
