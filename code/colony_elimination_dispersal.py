...
def simulate(J, S=20, p=2, c=0.1, N_ed=2, p_ed=0.25, N_re=4, N_c=100, rng=(-10,10), d_attract=0.1, w_attract=0.2, h_repellant=0.1, w_repellant=10):
    ...
    for l in range(N_ed):
        for k in range(N_re):
            ...
            I = np.argsort(J_histories[l, k].sum(axis=1)) # sort indices of thetas by cumulative values of J
            theta[I][S//2:] = theta[I][:S//2].copy() # replace sorted last half with sorted first half
        for i in range(S):
